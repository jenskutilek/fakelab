...
if TYPE_CHECKING:
    ...
    from .autohint import FontInstance
...
class FDDict:
    ...
    def setInfo(self, key, value) -> None:
        if key == "LanguageGroup":
            if (isinstance(value, numbers.Number) and value == 1) or value == "1":
                value = 1
            else:
                value = 0
...
def parseFontInfoFile(
    fdArrayMap: dict[int, FDDict],
    data: str,
    glyphList: list[str],
    maxY: int,
    minY: int,
    fontName: str,
) -> tuple[dict[str, int], FDDict | None]:
                ...
                if token == kFDDictToken:
                    ...
                    if dictName == kFinalDictName:
                        ...
                        if finalFDict is None:
                            finalFDict = FDDict(maxfdIndex + 1, dictName=dictName)
                        fdDict = finalFDict
                    else:
    ...
    for dictName, fdIndex in fdIndexDict.items():
        ...
        if fdDict.DominantH is None:
            log.warning("The FDDict '%s' in fontinfo has no DominantH value", dictName)
        if fdDict.DominantV is None:
            log.warning("The FDDict '%s' in fontinfo has no DominantV value", dictName)
    ...
    if fdIndexDict:
        ...
        defaultFDDict.BaselineOvershoot = 0  # type: ignore[attr-defined]
        defaultFDDict.CapHeight = maxY + 100  # type: ignore[attr-defined]
        defaultFDDict.CapOvershoot = 0  # type: ignore[attr-defined]
    ...
    for ki in [0, 1]:
        ...
        for zone in zoneList[1:]:
            ...
            if zone[1] < prevZone[0]:
                log.warning(
                    "For final FontDict, skipping zone %s in FDDict %s"
        ...
        for stem in stemList[1:]:
            if abs(stem - prevStem) < 2:
                log.warning(
                    "For final FontDict, skipping stem width %s in "
...
def fontinfoFileData(
    options: "HintOptions | ReportOptions", font: "UFOFontData | CFFFontData"
...
def getFDInfo(
    ...
    if hasDict and not options.ignoreFontinfo:
        ...
        fdDict = font.getPrivateFDDict(
            options.allowNoBlues,
            options.noFlex,
        ...
        minY, maxY = font.get_min_max(int(fdDict.OrigEmSqUnits))
        assert srcFontinfoData is not None
        fdSelectMap, finalFDict = parseFontInfoFile(
            fdArrayMap, srcFontinfoData, glyphList, int(maxY), int(minY), desc
        )
        if finalFDict is None:
        ...
        for name in glyphList:
            ...
            if fdIndex not in fdArraySet:
                ...
                if model not in dictRecord:
                    dictRecord[model] = {}
                dictRecord[model][fdIndex] = fddict  # type: ignore
...
class FDDictManager:
    def __init__(
        ...
        self.isVF = isVF
        self.fdSelectMap: dict | None = None
        self.auxRecord: dict[int, Any] = {}
        ...
        for i in fontInstances:
            (fdSelectMap, fdArrayMap, privateMap) = getFDInfo(
                i.font, i.font.desc, options, glyphList, isVF
            )
            if isVF:
    ...
    def getRecKey(self, gname: str, vsindex):
        ...
        fddict, _ = f.font.getPrivateFDDict(
            self.options.allowNoBlues,
            self.options.noFlex,
    ...
    def checkGlyphList(self) -> None:
        ...
        for label, charDict in [
            ("hCounterGlyphs", options.hCounterGlyphs),
