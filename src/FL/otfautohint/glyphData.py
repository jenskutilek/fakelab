...
class boundsState:
    ...
    def __init__(self, c: "pathElement") -> None:
        ...
        if c.is_line:
            ...
            self.mergePt(cpb, c.cs, 0.25, doExt=False)  # t value isn't used
            self.mergePt(cpb, c.ce, 0.75, doExt=False)  # t value isn't used
            if self.lb != cpb:
    ...
    def calcCurveBounds(self, pe) -> None:
        ...
        for dim in range(2):
            roots += [
                t for t in solveQuadratic(a[dim] * 3, b[dim] * 2, c[dim]) if 0 <= t < 1
            ]
    ...
    def intersects(self, other, margin: int = 0):
        ...
        """
        return (
            self.bounds[0][0] <= other.bounds[1][0] + margin
            and self.bounds[1][0] + margin >= other.bounds[0][0]
            and self.bounds[0][1] <= other.bounds[1][1] + margin
            and self.bounds[1][1] + margin >= other.bounds[0][1]
        )
...
class pathBoundsState:
    ...
    def within(self, other):
        ...
        """
        return (
            self.bounds[0][0] >= other.bounds[0][0]
            and self.bounds[0][1] >= other.bounds[0][1]
            and self.bounds[1][0] <= other.bounds[1][0]
            and self.bounds[1][1] <= other.bounds[1][1]
        )
...
class pathElement:
    ...
    def splitAt(self, t: "Number") -> Self:
        if self.is_line:
            ...
            ret = pathElement(
                pb, self.e, position=self.position, is_close=self.is_close
            )
            self.e = pb
            self.is_close = False
            self.bounds = None
...
class glyphData(BasePen):
    ...
    def hStem(self, data, is_hm: bool) -> None:
        ...
        if self.is_hm is not None and self.is_hm != is_hm:
            ...
            log.info(
                "Horizontal stem hints mask setting does not match"
                + " previous setting"
            )
        self.is_hm = is_hm
    ...
    def vStem(self, data, is_hm: bool | None) -> None:
        ...
        if is_hm is not None:
            if self.is_hm is not None and self.is_hm != is_hm:
                ...
                # using existing hint data
                log.info(
                    "Vertical stem hints mask setting does not match"
                    + " previous setting"
                )
            self.is_hm = is_hm
    ...
    def hintmask(self, hhints, vhints) -> None:
        ...
        if not self.is_hm:
            ...
            # using existing hint data
            log.info(
                "Hintmask found when stems weren't marked as having" + " hintmasks"
            )
            self.is_hm = True
    ...
    def hasHints(
        self, doVert: bool = False, both: bool = False, either: bool = False
    ) -> bool:
        """
    ...
    def inSubpath(self, c, i, skipTiny, closeWrapOK, segSub):
        ...
        while True:
            ...
            if not skipTiny or not c.isTiny():
                break
            if not closeWrapOK and ((i > 0 and o < offset) or (i < 0 and o > offset)):
                return None
        return c
    ...
    def checkAssocPoint(
        self, segs, spe, ope, sp, op, mapEnd, loose, factor=None
    ) -> bool:
        if factor is None:
            factor = ope.getAssocFactor(loose)
        if sp == op or ope.containsPoint(sp, factor):
            if not ope.containsPoint(spe.atT(0.5), factor * ope.middleMult):
                return False
            spe.association = ope
            return True
        else:
            ...
            if does and spe.tSlop < t < 1 - spe.tSlop:
                ...
                if not ope.containsPoint(spe.atT(midMapped), factor * ope.middleMult):
                    return False
    ...
    def associatePath(self, orig, loose: bool = False) -> None:
        ...
        while segs:
            ...
            if oepel:
                for oepe in oepel:
                    if self.checkAssocPoint(segs, c, oepe, c.s, oepe.s, True, loose):
                        done = True
                        break
                    else:
                        oepen = orig.nextInSubpath(oepe)
                        if self.checkAssocPoint(
                            segs, c, oepen, c.s, oepen.e, True, loose
                        ):
                            done = True
                            break
                if done:
            ...
            if ospel:
                for ospe in ospel:
                    ...
                    if self.checkAssocPoint(segs, c, ospen, c.e, ospen.e, False, loose):
                        done = True
                        break
                    elif self.checkAssocPoint(segs, c, ospe, c.e, ospe.s, False, loose):
                        done = True
                        break
                if done:
                    continue
            ...
            for oc in orig:
                ...
                if cBounds.intersects(oc.getBounds(), factor):
                    if oc.containsPoint(c.s, factor) and (
                        self.checkAssocPoint(
                            segs, c, oc, c.e, oc.e, False, loose, factor
                        )
                        or self.checkAssocPoint(
                            segs, c, oc, c.e, oc.s, False, loose, factor
                        )
                    ):
                        done = True
                        break
            ...
            for oc in orig:
                ...
                if cBounds.intersects(oc.getBounds(), factor):
                    if oc.containsPoint(c.e, factor) and (
                        self.checkAssocPoint(
                            segs, c, oc, c.s, oc.s, True, loose, factor
                        )
                        or self.checkAssocPoint(
                            segs, c, oc, c.s, oc.e, True, loose, factor
                        )
                    ):
                        done = True
                        break
    ...
    def addNullClose(self, si) -> None:
        ...
        pe = pathElement(sp[0].s, sp[0].s, is_close=True, position=(si, len(sp)))
        sp.append(pe)
