...
class dimensionHinter(ABC):
    ...
    class glIter:
        ...
        def __next__(self) -> list[GlyphPE]:
            return [GlyphPE(self.gll[i], ii.__next__()) for i, ii in enumerate(self.il)]
    ...
    def tryFlex(self, gl, c) -> bool:
        ...
        if real_n is not n:
            log.info("Remove short spline at {:g} {:g} to add flex.".format(*c.e))
            return False
        elif real_n.isClose() or real_n.isStart():
            log.info("Move closepath from {:g} {:.0g}to add flex.".format(*c.e))
            return False

        if fne(c.s.a, n.e.a):
            log.info(
                "Curves from {:g} {:g} to {:g} {:g} ".format(*c.s, *n.e)
        ...
        if (
            feq(n.s.a, n.cs.a)
            and feq(n.cs.a, n.ce.a)
            and feq(n.ce.a, n.e.a)
            and not self.linearFlexOK()
        ):
            log.info(
                "Make curves from {:g} {:g} to {:g} {:g}".format(*c.s, *n.e)
                + "non-linear to add flex"
            )  # XXX what if only one line?
            return False
    ...
    def calcHintValues(self, lnks, force=True, tryCounter=True) -> None:
        ...
        if self.glyph.hasHints(doVert=self.isV()):
            if force:
                log.info(f"Clearing existing {self.aDesc()} hints")
                self.glyph.clearHints(self.isV())
                self.glyph.changed = True
            else:
                self.keepHints = True
                log.info(f"Already has {self.aDesc()} hints")
                self.stopHint()
                return
        self.keepHints = False
        self.BigDist = max(self.dominantStems() + [self.InitBigDist])
        self.BigDist *= self.BigDistFactor
        log.debug(f"generate {self.aDesc()} segments")
        self.genSegs()
        self.limitSegs()
        log.debug(f"generate {self.aDesc()} stem values")
        ...
        log.debug(f"pick main {self.aDesc()}")
        ...
        if tryCounter and self.isCounterGlyph():
            ...
            if not self.hs.counterHinted:
                ...
                if not self.hs.counterHinted:
                    log.info(
                        (
                            "Glyph is in list for using %s counter hints "
        ...
        if len(self.hs.mainValues) == 0:
            self.addBBox(False)
        log.debug(f"{self.aDesc()} results")
        if self.hs.counterHinted:
            log.debug(f"Using {self.aDesc()} counter hints.")
    ...
    def addSegment(
        ...
        if pe1 is not None and hasattr(pe1, "association"):
            ...
            if (
                not mid1
                and ope1 is not None
                and not pe1.e.__eq__(ope1.e, ope1.getAssocFactor())
            ):
                mid1 = True
            pe1 = ope1
        if pe2 is not None and hasattr(pe2, "association"):
            ...
            if (
                not mid2
                and ope2 is not None
                and not pe2.e.__eq__(ope2.e, ope2.getAssocFactor())
            ):
                mid2 = True
            pe2 = ope2
        ...
        self.hs.addSegment(
            fr, to, loc, pe1, pe2, typ, self.Bonus, self.isV(), mid1, mid2, desc
        )
    ...
    def flatQuo(self, p1, p2, doOppo=False) -> Any:
        ...
        if q < 0.25:
            ...
            result = self.interpolate(q, 0.5, 1, 0.25, 2)
        elif q < 4:
    ...
    def relPosition(self, c, lower=False) -> bool:
        ...
        for subp in self.glyph.subpaths:
            if (lower and subp[0].s.a < c.s.a) or (not lower and subp[0].s.a > c.s.a):
                return True
        return False
    ...
    def doBendsNext(self, c) -> None:
        ...
        if osame or (
            ...
            and (
                ...
                or (self.flatQuo(p0, p1, doOppo=True) > 0 and self.testBend(p0, p1, p2))
            )
        ):
            ...
            self.addSegment(
                strt, end, p1.o, c, None, hintSegment.sType.BEND, "next bend forward"
            )
            if doboth:
                self.addSegment(
                    end,
    ...
    def doBendsPrev(self, c) -> None:
        ...
        if osame or (
            ...
            and (
                ...
                or (self.flatQuo(p1, p0, doOppo=True) > 0 and self.testBend(p2, p0, p1))
            )
        ):
            delta = self.BendLength / 2
            ...
            self.addSegment(
                strt, end, p0.o, cs, None, hintSegment.sType.BEND, "prev bend forward"
            )
    ...
    def sameDir(self, c, doPrev=False) -> bool | None:
        ...
        if self.diffSign(p0.y - p1.y, p1.y - p2.y) or self.diffSign(
            p0.x - p1.x, p1.x - p2.x
        ):
            return False
    ...
    def cpDirection(self, p0, p1, p2) -> int:
        ...
        """
        det = p0.x * (p1.y - p2.y) + p1.x * (p2.y - p0.y) + p2.x * (p0.y - p1.y)
        if det > 0:
            return 1
        elif det < 0:
            return -1
    ...
    def prepForSegs(self) -> None:
        for c in self.glyph:
            if not c.isLine() and (
                self.cpDirection(c.s, c.cs, c.ce) != self.cpDirection(c.cs, c.ce, c.e)
            ):
    ...
    def genSegs(self) -> None:
        ...
        if self.handleOverlap():
            ...
            if self.glyph is None:
                ...
                self.glyph.associatePath(origGlyph, self.options.looseOverlapMapping)
    ...
    def genSegsForPathElement(self, c) -> None:
        ...
        log.debug(f"Element {c.position[0]} {c.position[1] + 1}")
        if c.isStart():
            ...
            self.Bonus = 0
            if self.isSpecial(lower=False) and self.relPosition(c, lower=False):
                self.Bonus = self.SpecialCharBonus
            elif self.isSpecial(lower=True) and self.relPosition(c, lower=True):
                self.Bonus = self.SpecialCharBonus
        if c.isLine() and not c.isTiny():
            ...
            if q > 0:
                if feq(c.s.o, c.e.o):
                    ...
                    self.addSegment(
                        ...
                        prv,
                        c,
                        hintSegment.sType.LINE,
                        "line",
                    )
                    d = (c.s - c.e).abs()
                    if d.o <= 2 and (d.a > 10 or d.normsq() > 100):
                        log.info(
                            "The line from {:g} {:g} to {:g} {:g}".format(*c.s, *c.e)
                            + f" is not exactly {self.aDesc()}"
                        )
            else:
            ...
            if c.flex == 2:
                ...
                if self.flatQuo(prv.s, c.e) > 0:
                    self.addSegment(
                        prv.s.a, c.e.a, c.e.o, fl1prv, c, hintSegment.sType.LINE, "flex"
                    )
            if c.flex != 2:
                ...
                if q == 0:
                    ...
                    if feq(c.cs.o, c.s.o) or (
                        fne(c.ce.o, c.e.o)
                        and (
                            self.nodeIsFlat(c, doPrev=True)
                            or not self.sameDir(c, doPrev=True)
                        )
                    ):
                        q2 = self.flatQuo(c.ce, c.s)
                        if (
                            q2 > 0
                            and self.sameSign(c.cs.a - c.s.a, c.ce.a - c.s.a)
                            and abs(c.ce.a - c.s.a) > abs(c.cs.a - c.s.a)
                        ):
                            adist = self.adjustDist(
                                self.CPTo(c.cs.a, c.ce.a) - c.s.a, q2
                            )
            ...
            if c.flex != 1:
                ...
                if q == 0:
                    ...
                    fne(c.cs.o, c.s.o)
                    and (
                        self.nodeIsFlat(c, doPrev=False)
                        or not self.sameDir(c, doPrev=False)
                    )
                ):
                    ...
                    if q2 > 0 and abs(ad2) > abs(adist):
                        ...
                        if (
                            feq(c.s.o, c.cs.o)
                            and feq(c.cs.o, c.ce.o)
                            and feq(c.ce.o, c.e.o)
                        ):
                            if (
                                self.options.allowChanges
                                and self.AutoLinearCurveFix
                                and not self.isMulti
                            ):
                                c.convertToLine()
                                ccs, cce = c.s, c.e
                                log.info(
                                    f"Curve from {c.s} to {c.e} changed to a line."
                                )
                            elif not self.isMulti:
                                log.info(
                                    f"Curve from {c.s} to {c.e} can be changed to a line."
                                )
                        adist = ad2 / 2
                        aavg = (c.s.a + c.e.a) / 2
                        sp = self.pickSpot(
                        ...
                        if (
                            q2 > 0
                            and self.sameSign(c.cs.a - c.e.a, c.ce.a - c.e.a)
                            and abs(c.ce.a - c.e.a) < abs(c.cs.a - c.e.a)
                        ):
                            aend = self.adjustDist(
                                c.e.a - self.CPFrom(c.cs.a, c.ce.a), q2
                            )
    ...
    def limitSegs(self) -> None:
        ...
        if not self.options.explicitGlyphs and maxsegs > self.options.maxSegments:
            log.warning(
                f"Calculated {maxsegs} segments, skipping {self.aDesc()} stem testing"
            )
    ...
    def showSegs(self) -> None:
        ...
        for pe in self.glyph:
            log.debug(f"for path element x {pe.e.x:g} y {pe.e.y:g}")
    ...
    def genStemVals(self) -> None:
        ...
        if self.hasBands():
            ghostSeg = hintSegment(
                0, 0, 0, None, hintSegment.sType.GHOST, 0, self.isV(), False, "ghost"
            )
            for s in ll:
    ...
    def evalPair(self, ls, us) -> tuple[Any, int]:
        ...
        if us.min <= ls.max and us.max >= ls.min:  # overlap
            ...
            if feq(overlen, minlen):
                ...
                dist = loc_d * (1 + 0.4 * (1 - overlen / minlen))
        else:
            o_d = min(abs(us.min - ls.max), abs(us.max - ls.min))
            dist = round(loc_d * self.NoOverlapPenalty + o_d * o_d / self.GapDistDenom)
    ...
    def stemMiss(self, ls, us) -> int | None:
        ...
        if d == 0 or d > 2:
            return None
        log.info(
            "{} {} stem near miss: {:g} instead of {:g} at {:g} to {:g}.".format(
                self.aDesc(),
    ...
    def insertStemValue(self, sv, note="add") -> None:
        ...
        if sv.isGhost:
            ...
            while j < len(svl) and svl[j] == sv:
                if (
                    not svl[j].isGhost
                    and (svl[j].lseg == sv.lseg or svl[j].useg == sv.useg)
                    and svl[j].val > sv.val
                ):
                    # Don't add
                    return
                j += 1
    ...
    def pruneStemVals(self) -> None:
        ...
        """
        ignorePruned = not self.isV() and (
            self.options.report_zones or not self.options.report_stems
        )
        svl = self.hs.stemValues
        for c in svl:
            ...
            for d in svl:
                if d.pruned and not ignorePruned:
                    continue
                if not c.isGhost and d.isGhost and not d.val > c.val * self.VeryMuchPF:
                    continue
                ...
                if c.val < 100 and d.val > c.val * self.MuchPF:
                    ...
                    cs_tst = csl and csu
                if (
                    d.val > c.val * self.PruneFactor
                    and c.lloc - self.PruneDistance <= d.lloc
                    and c.uloc + self.PruneDistance >= d.uloc
                    and cs_tst
                    and (
                        self.isV()
                        or c.val < 16
                        or (
                            (not uInBand or feq(c.uloc, d.uloc))
                            and (not lInBand or feq(c.lloc, d.lloc))
                        )
                    )
                ):
                ...
                if c.lseg is not None and c.useg is not None:
                    if abs(c.lloc - d.lloc) < 1:
                        if (
                            not otherLow
                            and c.val < d.val * self.PruneFactorLt
                            and abs(d.uloc - d.lloc) < abs(c.uloc - c.lloc)
                            and csl
                        ):
                            otherLow = True
                        if (
                            (
                                self.isV()
                                or (d.val > c.val * self.PruneFactor and not uInBand)
                            )
                            and c.useg.isBend()
                            and csl
                        ):
                    ...
                    if abs(c.uloc - d.uloc) < 1:
                        ...
                        if (
                            (
                                self.isV()
                                or (d.val > c.val * self.PruneFactor and not lInBand)
                            )
                            and c.lseg.isBend()
                            and csu
                        ):
    ...
    def prune(self, sv, other_sv, desc) -> None:
        ...
        """
        log.debug(f"Prune {self.aDesc()} val: {desc}")
    ...
    def findHighestValForSegs(self, segl, isU) -> None:
        ...
        for seg in segl:
            ...
            if highest is not None:
                if not (
                    highest.val < self.MinHighSegVal
                    and (ghst is None or ghst.val < self.MinHighSegVal)
                ):
    ...
    def findHighestVal(self, seg, isU, locFlag) -> Any:
        ...
        def OKcond(sv):
            ...
            return (
                abs(vl - seg.loc) <= self.MaxMerge
                and loctest
                and self.considerValForSeg(sv, seg, isU)
            )
        ...
        try:
            ...
            pass
        log.debug(f"findHighestVal: loc {seg.loc:g} min {seg.min:g} max {seg.max:g}")
        if highest:
    ...
    def findBestValues(self) -> None:
        ...
        for i in range(svll):
            ...
            for j in range(i + 1, svll):
                if (
                    svl[j].best is not None
                    or fne(svl[j].lloc, svl[b].lloc)
                    or fne(svl[j].uloc, svl[b].lloc)
                ):
                    continue
                blst.append(svl[j])
                if (svl[j].spc == svl[b].spc and svl[j].val > svl[b].val) or svl[
                    j
                ].spc > svl[b].spc:
                    b = j
            for sv in blst:
    ...
    def replaceVals(self, oldl, oldu, newl, newu, newbest) -> None:
        ...
        for sv in self.hs.stemValues:
            if fne(sv.lloc, oldl) or fne(sv.uloc, oldu) or sv.merge:
                continue
            log.debug(
                f"Replace {self.aDesc()} hints pair at {oldl:g} {oldu:g} by {newl:g} {newu:g}"
            )
            log.debug(
                f"\told value {sv.val:g} {sv.spc:g} new value {newbest.val:g} {newbest.spc:g}"
            )
            sv.lloc = newl
            sv.uloc = newu
            sv.val = newbest.val
            ...
            for sv in svl:
                ...
                if (
                    (
                        ...
                        and self.closeSegs(bst.lseg, sv.lseg)
                        and not btlIB
                        and not svuIB
                        and not btuIB
                    )
                    or (
                        feq(bst.uloc, sv.uloc)
                        and self.closeSegs(bst.useg, sv.useg)
                        and not btuIB
                        and not svlIB
                        and not btlIB
                    )
                    or (
                        ...
                        and abs(bst.uloc - sv.uloc) <= self.MaxMerge
                        and (self.isV() or feq(bst.lloc, sv.lloc) or not svlIB)
                        and (self.isV() or feq(bst.uloc, sv.uloc) or not svuIB)
                    )
                ):
                    if (
                        feq(bst.best.spc, sv.spc)
                        and feq(bst.best.val, sv.val)
                        and not self.isV()
                    ):
                        if svlIB:
                            if bst.lloc > sv.lloc:
    ...
    def limitVals(self) -> None:
        ...
        log.info(
            f"Trimming stem list to {self.StemLimit:g} from {len(representatives):g} unique positions"
        )
    ...
    def checkVals(self) -> None:
        ...
        for sv in self.hs.stemValues:
            ...
            if fne(l, lPrev) or fne(u, uPrev):
                ...
                if not sv.isGhost:
                    log.info(
                        f"{self.aDesc()} {'linear' if line else 'curve'} stem near miss: "
                        f"{w:g} instead of {mdw:g} at {l:g} to {u:g}"
                    )
            lPrev, uPrev = l, u
    ...
    def mainVals(self) -> None:
        ...
        while True:
            ...
            if best.isGhost:
                for sv in svl:
                    if (
                        feq(best.lloc, sv.lloc)
                        and feq(best.uloc, sv.uloc)
                        and not sv.isGhost
                    ):
                ...
                if lseg.isGhost():
                    ...
                    if newbest is not None and newbest is not best and newbest in svl:
                        best = newbest
                elif useg.isGhost():
    ...
    def tryCounterHinting(self) -> bool:
        ...
        for hv in hvl[0:3]:
            ...
            if loc < minloc:
                ...
                maxdelta = delta
        if (
            abs(mindelta - maxdelta) < self.DeltaDiffMin
            and abs((maxloc - midloc) - (midloc - minloc)) < self.DeltaDiffMin
        ):
            self.hs.rejectValues.extend(hvl[3:])
            self.hs.mainValues = hvl[0:3]
            return True
        if (
            abs(mindelta - maxdelta) < self.DeltaDiffReport
            and abs((maxloc - midloc) - (midloc - minloc)) < self.DeltaDiffReport
        ):
            log.info(f"Near miss for {self.aDesc()} counter hints.")
        return False
    ...
    def addBBox(self, doSubpaths=False) -> None:
        ...
        for param in paraml:
            ...
            self.hs.mainValues.sort(
                key=lambda sv: sv.compVal(self.SpcBonus), reverse=True
            )
    ...
    def convertToStemLists(self) -> bool:
        ...
        for sv in self.hs.stemValues:
            ...
            if sidx is not None:
                sv.idx = sidx
                if self.options.removeConflicts:
                    wl[sidx] += sv.compVal(self.SpcBonus)[0] ** 0.25
            else:
                ...
                if self.options.removeConflicts:
                    ...
                    wl.append(sv.compVal(self.SpcBonus)[0] ** 0.25)
        ...
        log.debug(f"Initial {self.aDesc()} stem list (including regions, if any):")
        for sidx in range(len(self.hs.stems[0])):
            log.debug(f"Stem {sidx}: {[sl[sidx] for sl in self.hs.stems]}")
        self.stopStemConvert()
        return True
    ...
    def calcInstanceStems(self, glidx) -> None:
        ...
        iSSl = [
            (instanceStemState(s.lb, self), instanceStemState(s.rt, self))
                    ...
                    if seg0.isBBox():
                        ...
                        for posi, segil in peSi.segLists(pos0):
                            for segi in segil:
                                ...
                                iSS.addToLoc(
                                    segi.loc, score, strong=(posi == pos0), seg=segi
                                )
                                found = True
        ...
        for sidx, s0 in enumerate(hs0.stems[0]):
            ...
            if isG == "low":
                ...
                if hi < lo:
                    ...
                    lo = hi + 50
                    log.warning(
                        "Stem end is less than start for non-"
    ...
    def bestLocation(self, sidx, ul, iSSl, hs0: glyphHintState) -> Any:
        ...
        for sv in hs0.stemValues:
            ...
            if seg.hintval.idx != sidx:
                ...
                if loc is not None:
                    return loc
        side = "lower" if ul == 0 else "upper"
        log.warning(f"No data for {side} location of stem {sidx}")
        assert hs0.stems is not None
    ...
    def convertToMasks(self) -> None:
        ...
        if hasConflicts:
            _, self.hs.goodMask = self.unconflict(
                sc, self.hs.goodMask, [not a for a in self.hs.goodMask]
            )
            log.info(
                f"Removed {self.aDesc()} stems {[i for i, g in enumerate(self.hs.goodMask) if not g]} to resolve conflicts"
            )
    ...
    def makePEMask(self, pestate, c) -> None:
        ...
        for seg in segments:
            if (
                not seg.hintval
                or seg.hintval.idx is None
                or not self.hs.goodMask[seg.hintval.idx]
                or seg.suppressed
            ):
                continue
        ...
        if self.hs.hasOverlaps and True in mask:
            for sidx in range(l):
                for sjdx in range(sidx + 1, l):
                    if not (
                        mask[sidx] and mask[sjdx] and self.hs.stemOverlaps[sidx][sjdx]
                    ):
                        continue
                    remidx = None
                    _, segi = max(
                    ...
                    if vali.val < self.ConflictValMin and self.OKToRem(
                        segi.loc, vali.spc
                    ):
                        remidx = sidx
                    elif (
                        valj.val < self.ConflictValMin
                        and vali.val > valj.val * self.ConflictValMult
                        and self.OKToRem(segj.loc, valj.spc)
                    ):
                        remidx = sjdx
                    elif (
                        c.isLine()
                        or self.flatQuo(p0, p1) > 0
                        and self.OKToRem(segi.loc, vali.spc)
                    ):
                        remidx = sidx
                        ...
                        (feq(p1.o, valj.lloc) or feq(p1.o, valj.uloc))
                        and fne(p0.o, vali.lloc)
                        and fne(p0.o, vali.uloc)
                    ):
                        remidx = sidx
                    elif (
                        (feq(p0.o, vali.lloc) or feq(p0.o, vali.uloc))
                        and fne(p1.o, valj.lloc)
                        and fne(p1.o, valj.uloc)
                    ):
                        remidx = sjdx
                    # XXX ResolveConflictBySplit was called here.
                    if remidx is not None:
                        mask[remidx] = False
                        log.debug(f"Resolved conflicting hints at {c.e.x:g} {c.e.y:g}")
                    else:
    ...
    def OKToRem(self, loc, spc) -> bool:
        return spc == 0 or (not self.inBand(loc, False) and not self.inBand(loc, True))
    ...
    def checkNearBands(self, loc, pl) -> None:
        for p in pl:
            if loc >= p[1] - self.NearFuzz and loc < p[1]:
                log.info(
                    f"Near miss above horizontal zone at {loc:f} instead of {p[1]:f}."
                )
            if loc <= p[0] + self.NearFuzz and loc > p[0]:
                log.info(
                    f"Near miss below horizontal zone at {loc:f} instead of {p[0]:f}."
                )
...
class glyphHinter:
    ...
    def _hint(
        ...
        log.info(f"Begin {self.taskDesc} (using fdDict '{fddict.DictName}').")
    ...
    def compatiblePaths(
        ...
        for i, (g, fd) in enumerate(zip(gllist[1:], fddicts[1:])):
            ...
            if len(glyph.subpaths) != len(g.subpaths):
                log.warning(
                    f"Path has a different number of subpaths compared with '{fd.FontName}': Cannot hint."
                )
                return False
            for si, dp in enumerate(glyph.subpaths):
                ...
                if gpl != dpl:
                    if gpl == dpl + 1 and gp[-1].isClose() and not dp[-1].isClose():
                        for _gi in range(i + 1):
                            gllist[_gi].addNullClose(si)
                        continue
                    if dpl == gpl + 1 and dp[-1].isClose() and not gp[-1].isClose():
                        g.addNullClose(si)
                        continue
                    log.warning(
                        f"subpath {si} has {len(gp)} elements while '{fd.FontName}' had {len(dp)} elements: Cannot hint"
                    )
                    return False
        ...
        for c in self.hHinter:
            ...
            if any(isline) and not all(isline):
                if self.options.allowChanges:
                    ...
                    log.warning(
                        "Mixed lines and curves without " + "allow-changes: Cannot hint"
                    )
                    return False
        return True
    ...
    def distributeMasks(self, glyph: glyphData) -> list | None:
        ...
        if glyph.hhs.counterHinted:
            usedmasks[0] = [
                mv or uv for mv, uv in zip(glyph.hhs.mainMask, usedmasks[0])
            ]
        if glyph.vhs.counterHinted:
            usedmasks[1] = [
                mv or uv for mv, uv in zip(glyph.vhs.mainMask, usedmasks[1])
            ]
        ...
        while c:
            ...
            if conflict:
                ...
                masks[:] = candmasks
                if mode == SHORT:
                    incompatmasks, _ = self.joinMasks(incompatmasks, cmasks, False)
    ...
    def bridgeMasks(self, glyph, o, n, used, pe) -> None:
        ...
        if self.mergeMain(glyph):
            no, _ = self.joinMasks(o, [glyph.hhs.mainMask, glyph.vhs.mainMask], False)
            o[:] = no
        for hv in range(2):
    ...
    def listHintInfo(self, glyph) -> None:
        ...
        for pe in glyph:
            ...
            if hList or vList:
                log.debug(f"hintlist x {pe.e.x:g} y {pe.e.y:g}")
                for seg in hList:
            ...
            while c != n and not cDone:
                ...
                for cseg in csl:
                    ...
                    for nseg in nsl:
                        ...
                        if diff != 0 and self.isUSeg(
                            cseg.loc, chv.uloc, chv.lloc
                        ) == self.isUSeg(nseg.loc, nhv.uloc, nhv.lloc):
                            if chv.compVal(self.hHinter.SpcBonus) > nhv.compVal(
                                self.hHinter.SpcBonus
                            ):
                                if nhv.spc == 0 and nhv.val < self.FlareValueLimit:
                                    self.reportRemFlare(n, c, "n")
                                    nseg.suppressed = True
                            else:
                                if chv.spc == 0 and chv.val < self.FlareValueLimit:
    ...
    def reportRemFlare(self, pe, pe2, desc) -> None:
        direction = "vertical" if self.doV else "horizontal"
        log.debug(
            f"Removed {direction} flare at {pe.e.x:g} {pe.e.y:g} by {pe2.e.x:g} {pe2.e.y:g} : {desc}"
        )
