...
def denormalizeValue(nv: float, a: Axis) -> float:
...
def denormalizeLocation(loc: dict[str, float], axes: list[Axis]) -> dict[str, float]:
    return {
        a.axisTag: denormalizeValue(loc.get(a.axisTag, a.defaultValue), a) for a in axes
    }
...
class T2ToGlyphDataExtractor(T2OutlineExtractor):
    ...
    def __init__(
        ...
        T2OutlineExtractor.__init__(
            self, gd, localSubrs, globalSubrs, nominalWidthX, defaultWidthX
        )
        self.glyph = gd
        self.readStems = readStems
        self.readFlex = readFlex
        self.hintMaskBytes: int | None = None
        self.vhintCount = 0
        self.hhintCount = 0
    ...
    def doMask(self, index, cntr=False):
        ...
        vhints = [hintOn(i, hintMaskString) for i in range(self.hhintCount, tc)]
...
def convertT2ToGlyphData(
    t2CharString, readStems=True, readFlex=True, roundCoords=True, name=None
):
    ...
    extractor = T2ToGlyphDataExtractor(
        gl,
        subrs,
...
def _run_tx(args: list[str]) -> None:
    try:
...
class CFFFontData:
    ...
    def __init__(self, path: str, font_format: str) -> None:
        ...
        self.is_cff2 = False
        self.is_vf = False
        self.vs_data_models: list[VarDataModel] | None = None
        self.desc = None
        if font_format == "OTF":
    ...
    def get_min_max(self, upm: int) -> tuple[int, int]:
        if self.is_cff2 and "hhea" in self.ttFont:
            ...
            font_max = int(upm * 1.25)
            font_min = int(-upm * 0.25)
    ...
    def convertToGlyphData(
        ...
        try:
            gl = convertT2ToGlyphData(
                t2CharString, readStems, readFlex, roundCoords, name=glyphName
            )
        except SEACError:
            log.warning("Skipping %s: can't process SEAC composite glyphs.", glyphName)
            gl = None
        return gl

    def updateFromGlyph(self, gl: glyphData | None, glyphName: str) -> None:
        if gl is None:
            return
        t2Program = gl.T2()
    ...
    def getPrivateDictVal(
        self,
        pDict: PrivateDict,
        attr: str,
        default: int | list,
        vsindex: int | None,
        vsi: VarStoreInstancer | None,
    ) -> int | float | list[float] | list[int]:
        ...
        if attr in (
            ...
            last_calc = 0
            vlist: list[Any] = []
            for dl in val:  # type: ignore[union-attr]
                if isinstance(dl, list):
                    assert vsi is not None
                    calc = dl[0] - last_def + vsi.interpolateFromDeltas(vsindex, dl[1:])
    ...
    def getPrivateFDDict(
        ...
        desc: str,
        fdIndex: int = 0,
        gl_vsindex: int | None = None,
    ) -> tuple[list[fdTools.FDDict], int] | fdTools.FDDict:
        ...
        for vn, vsi in enumerate(vsi_list):
            ...
            fdDict.setInfo(
                ...
                self.getPrivateDictVal(
                    privateDict, "LanguageGroup", 0, dict_vsindex, vsi
                ),
            )
            ...
            for bvattr, bvkeys in (
                ("BlueValues", fdTools.kBlueValueKeys),
                ("OtherBlues", fdTools.kOtherBlueValueKeys),
            ):
                bvs = self.getPrivateDictVal(privateDict, bvattr, [], dict_vsindex, vsi)
                numbvs = len(bvs)  # type: ignore[arg-type]
                if bvattr == "BlueValues" and numbvs < 4:
                    low, high = self.get_min_max(upm)
                    low = int(min(-upm * 0.25, low))
                    high = int(max(upm * 1.25, high))
                ...
                for i in range(numbvs):
                    key = bvkeys[i]
                    value = bvs[i]  # type: ignore[index]
                    fdDict.setInfo(key, value)

            for ssnap, stdw, fdkey in (
                ("StemSnapV", "StdVW", "DominantV"),
                ("StemSnapH", "StdHW", "DominantH"),
            ):
                if hasattr(privateDict, ssnap):
                    sstems = self.getPrivateDictVal(
                        privateDict, ssnap, [], dict_vsindex, vsi
                    )
                elif hasattr(privateDict, stdw):
                    sstems = [  # type: ignore[assignment]
                        self.getPrivateDictVal(privateDict, stdw, -1, dict_vsindex, vsi)
                    ]
                else:
                    if allowNoBlues or self.is_vf:
                        # XXX adjusted for vf
            ...
            fdDict.setInfo(
                "BlueFuzz",
                self.getPrivateDictVal(privateDict, "BlueFuzz", 1, dict_vsindex, vsi),
            )
    ...
    def getPrivateDict(self, fdIndex: int) -> PrivateDict:
    ...
    def getInstanceName(vn: int, vsi: VarStoreInstancer, axes: list[Axis]) -> str:
        if vn == 0:
            ...
            vals = (f"{v:g}" for v in denormalizeLocation(vsi.location, axes).values())
        return "{" + ",".join(vals) + "}"

    def get_vf_glyphs(self, glyph_name: str) -> tuple[list[glyphData], int]:
        ...
        for vsi in vs_data_model.master_vsi_list:
            ...
            t2_program = interpolate_cff2_charstring(
                charstring, glyph_name, iFD, vsindex
            )
            self.temp_cs_extract.program = t2_program
            glyph = convertT2ToGlyphData(
                self.temp_cs_extract, True, False, True, name=glyph_name
            )
    ...
    def get_vs_data_models(topDict: TopDict, axes: list[Axis]) -> list[VarDataModel]:
        ...
        for vsindex, var_data in enumerate(otvs.VarData):
            ...
            for region_idx in var_data.VarRegionIndex:
                ...
                vsi = VarStoreInstancer(topDict.VarStore.otVarStore, axes, loc)
    ...
    def merge_hinted_glyphs(self, name: str) -> None:
        vsindex, addIndex = self.vsIndexMap[name]
        assert self.vs_data_models is not None
        new_t2cs = merge_hinted_programs(
            self.temp_cs_merge, self.glyph_programs, name, self.vs_data_models[vsindex]
        )
        if addIndex:
