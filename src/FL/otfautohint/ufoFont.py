...
class UFOFontData:
    ...
    def save(self, path: str) -> None:
        ...
        if os.path.abspath(self.path) != os.path.abspath(path):
            ...
            log.info(
                "Copying from source UFO font to output UFO font before processing..."
            )
            if os.path.exists(path):
                shutil.rmtree(path)
            shutil.copytree(self.path, path)

        writer = UFOWriter(path, self._reader.formatVersionTuple, validate=False)
        ...
        for name, glyph in self.newGlyphMap.items():
            ...
            if not self.writeToDefaultLayer and name in self.processedLayerGlyphMap:
        ...
        if self._hashmap is None:
            ...
            if version[0] > HASHMAP_VERSION[0]:
                raise FontParseError(
                    "Hash map version is newer than otfautohint. Please update."
                )
    ...
    def _get_or_skip_glyph(self, name, readStems, readFlex, roundCoords, doAll):
        ...
        glyph = glyphset[name]
        glyph_data = self.get_glyph_data(
            glyph, name, readStems, readFlex, roundCoords, glyphset
        )
    ...
    def getPrivateFDDict(
        ...
        fdDict.setInfo("LanguageGroup", self.fontInfo.get("languagegroup", "0"))
        upm = self.getUnitsPerEm()
        fdDict.setInfo("OrigEmSqUnits", upm)
        low = min(-upm * 0.25, self.fontInfo.get("openTypeOS2WinDescent", 0) - 200)
        high = max(upm * 1.25, self.fontInfo.get("openTypeOS2WinAscent", 0) + 200)
