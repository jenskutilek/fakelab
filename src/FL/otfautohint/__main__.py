    ...
    def _split_lines(self, arg, width):
        ...
        for i, line in enumerate(arg_rows):
            ...
            if line.strip() == "":
                ...
                lines = [
                    self.__add_whitespace(j, line_wtsp, x)
...
def _validate_font_paths(path_lst, parser):
    ...
    for path in path_lst:
        ...
        if font_format in ("PFA", "PFB", "PFC"):
            ...
            if not has_tx:
                parser.error(
                    f"{base_name} font format requires 'tx'. Please install 'afdko'."
                )
...
RE_COUNTER_CHARS = re.compile(r"([VH]CounterChars)\s+[\(\[]\s*([^\)\r\n]+)[\)\]]")
...
def add_common_options(parser, term, name):
    ...
    parser.add_argument(
        ...
        "--processes",
        type=int,
        help="The number of glyph-%sing processes (default is "
        % term
...
def handle_glyph_lists(options, parsed_args):
    if parsed_args.glyphs_to_hint:
        ...
        options.explicitGlyphs = True
        options.glyphList = _process_glyph_list_arg(
            _split_comma_sequence(_read_txt_file(parsed_args.glyphs_to_hint_file)),
            options.nameAliases,
        )
    elif parsed_args.glyphs_to_not_hint_file:
        options.excludeGlyphList = True
        options.glyphList = _process_glyph_list_arg(
            _split_comma_sequence(_read_txt_file(parsed_args.glyphs_to_not_hint_file)),
            options.nameAliases,
        )
    ...
    if parsed_args.overlaps_to_hint:
        ...
        options.overlapList = _process_glyph_list_arg(
            _read_txt_file(parsed_args.overlaps_to_hint_file), options.nameAliases
        )


def get_options(args):
    parser = argparse.ArgumentParser(
        formatter_class=_CustomHelpFormatter, description=__doc__
    )
    ...
    parser.add_argument(
        "-d", "--decimal", action="store_true", help="use decimal coordinates"
    )
    parser.add_argument(
        "--no-flex", action="store_true", help="suppress generation of flex commands"
    )
    parser.add_argument(
        "--no-hint-sub", action="store_true", help="suppress hint substitution"
    )
    parser.add_argument(
        "--no-zones-stems",
        action="store_true",
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        type=_validate_path,
        help=f"file containing hinting parameters\nDefault: '{FONTINFO_FILE_NAME}'",
    )
    parser.add_argument(
        "--ignore-fontinfo",
        action="store_true",
    ...
    if not len(parsed_args.font_paths or []) and len(parsed_args.output_paths or []):
    ...
    if not len(parsed_args.font_paths):
        parser.error(f"the following arguments are required: {parser_fonts.metavar}")

    if parsed_args.output_paths and len(parsed_args.font_paths) != len(
        parsed_args.output_paths
    ):
    ...
    if options.ignoreFontinfo:
        ...
        fontinfo_path = os.path.join(
            os.path.dirname(all_font_paths[0]), FONTINFO_FILE_NAME
        )
