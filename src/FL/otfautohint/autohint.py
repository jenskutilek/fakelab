...
from .report import GlyphReport, Report
...
class ACOptions(object):
    def __init__(self) -> None:
        ...
        self.vCounterGlyphs = {"m": False, "M": False, "T": False, "ellipsis": False}
        self.upperSpecials = {
            "questiondown": False,
            "exclamdown": False,
            "semicolon": False,
        }
        self.lowerSpecials = {"question": False, "exclam": False, "colon": False}
        self.noBlues = {
            "at": False,
            "bullet": False,
            "copyright": False,
...
def getGlyphNames(
    glyphSpec: str, fontGlyphList: list[str], fDesc: str
) -> list[str] | None:
    ...
    if glyphSpec.isnumeric():
        ...
        if GID >= len(fontGlyphList):
            log.warning(
                "glyph ID <%s> from glyph selection list option "
    ...
    if len(rangeList) == 1:
        ...
        log.warning(
            "glyph name <%s> from glyph selection list option is not in font <%s>",
            glyphSpec,
            fDesc,
        )
        return None
    ...
    for r in rangeList:
        if r.isnumeric():
            ...
            if GID >= len(fontGlyphList):
                log.warning(
                    "glyph name <%s> in range %s from glyph "
...
def filterGlyphList(
    options: "HintOptions | ReportOptions", fontGlyphList: list[str], fDesc: str
) -> list[str]:
    """
...
def get_glyph(
    options: "HintOptions | ReportOptions", font: UFOFontData | CFFFontData, name: str
) -> glyphData | None:
    try:
        gl = font.convertToGlyphData(
            name,
            options.readHints,  # stems
...
class fontWrapper:
    ...
    def __init__(
        self, options: "HintOptions | ReportOptions", fil: list[FontInstance]
    ) -> None:
        ...
        if len(fil) > 1:
            ...
            self.isVF = hasattr(fil[0].font, "ttFont") and "fvar" in fil[0].font.ttFont
        self.reportOnly = options.justReporting()
        assert not self.reportOnly or (not self.isVF and len(fil) == 1)
        self.notFound = 0
        self.glyphNameList = filterGlyphList(
            options, fil[0].font.getGlyphList(), fil[0].font.desc
        )
    ...
    def hint(self) -> bool:
        ...
        if pcount > 1 and current_process().daemon:
            ...
            if self.options.process_count is not None:
                log.warning(
                    "Program was already spawned by a different "
        ...
        try:
            ...
            if pcount == 1:
                ...
                pool = Pool(
                    pcount,
                    initializer=glyphHinter.initialize,
                    initargs=(self.options, dictRecord, logQueue),
                )
            ...
            if self.notFound:
                log.info("Skipped %s of %s glyphs.", self.notFound, self.numGlyphs())
...
def openFont(
    path: str, options: "HintOptions | ReportOptions"
...
def get_outpath(
    options: "HintOptions | ReportOptions", font_path: str, i: int | str
) -> str:
    if i == "r":
...
def hintFiles(options: "HintOptions | ReportOptions") -> None:
    ...
    if options.referenceFont:
        ...
        if hasattr(font, "ttFont"):
            assert "fvar" not in font.ttFont, (
                "Can't use a CFF2 VF font as a default font in a set of MM fonts."
            )
        fontInstances.append(
            FontInstance(
                font,
    ...
    if fontInstances and options.referenceFont:
        log.info(
            "Hinting fonts with reference %s. Start time: %s.",
            fontInstances[0].font.desc,
            time.asctime(),
        )
        ...
        if fw.hint():
            ...
            fw.close()
        log.info(
            "Done hinting fonts with reference %s. End time: %s.",
            fontInstances[0].font.desc,
            time.asctime(),
        )
    else:
        for fi in fontInstances:
            log.info("Hinting font %s. Start time: %s.", fi.font.desc, time.asctime())
            if fi[0].isCID():
            ...
            if fw.hint():
                ...
                fw.close()
            log.info(
                "Done hinting font %s. End time: %s.", fi.font.desc, time.asctime()
            )
